@echo off
REM Builds digital_noise_config.exe with mingw-w64 gcc. Install it via MSYS2
...
gcc -std=c99 -Wall -Wextra -Wpedantic -Werror -mwindows -Os -s ^
    ...
    -o digital_noise_config.exe src\main.c src\connection.c src\device.c src\protocol.c src\serial_port.c src\app_res.o ^
...
echo Build OK: digital_noise_config.exe
